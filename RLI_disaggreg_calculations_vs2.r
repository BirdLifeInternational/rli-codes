...
repetitions=500     ## n repetitions - change to new max rep (was 1000, reduced to reduce processing time)
saveglobal=F           ### set as FALSE if no need to save global rlis (i.e., only to save the subglobal)
skip_subglobal=F           ### set as TRUE if no need to run subglobal rlis (i.e., only to run the global)
skip_thematic=T     # if only to run general RLIs (i.e., skip the thematic RLI)
only_thematic=F     # if only to run thematic RLIs (i.e., ignore the general RLI)
dis_subglobal=F     # if disaggregated RLIs to be calculated also at subglobal levels
...
isos2use=c("ISO_BL","ISO_SDG","SDG_Region","SDG_Subregion","LLDC_SIDS","LDC")   ### IMPORTANT NOTE!!! IF TO RUN BOTH ISO_BL AND ISO_SDG, ISO_BL SHOULD BE FIRST IN THE LIST    
#c("ISO_BL","ISO_SDG","SDG_Region","SDG_Subregion","LLDC_SIDS","LDC","IPBES_region","CMS_Region") 
...
driv_gc=list(utilisation=utilisation,pollution=pollution,IAS=IAS, fisheries=fisheries)
#driv_gc=NULL
...
#splists=c("internationallytraded","medicine","forest","wetland","migratory")  # internationallytraded	medicine	forest	wetland	migratory	marine	terrestrial	freshwater
...
#combs=data.frame(driv=c("utilisation", "pollution","fisheries"), spgroup=c("migratory","migratory","migratory"))  
#combs=NULL
...
for (k in mink:n_iter)  #### loop to go through all the disaggregations
{
 #k=2
  ...
  ### k=1 runs the general RLIs (all species and genuine changes)
  if (k ==1){       
   ...
   if (k>1&k<(length(driv_gc)+2)) 
    {   
        ...
        nrow(gench_alt)
        tbf1=sppp1
        finfolder=paste(finfolderP,name2save, sep="/")
        finfolder
        print(k)
        print(finfolder)
...
finalname
if (saveglobal) write.csv(final2save,finalname, row.names=F)
...
finalname
if (saveglobal) write.csv(final2save2,finalname, row.names=F)
  ...
  #for (x in 1:4)
{
  ...
  final2save=final[, c("country","group","year","rli","qn05","qn95")]
